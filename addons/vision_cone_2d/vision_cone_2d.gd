...
func ray_collide(direction: Vector2) -> Dictionary:
	# TODO add offset to origin
	var destination = global_position + direction
	var query = PhysicsRayQueryParameters2D.create(global_position, destination, collision_layer_mask)
	var collision = get_world_2d().direct_space_state.intersect_ray(query)
...
func collide() -> Array:
	var collisions = []
	for i in range(ray_count + 1): 
		var col = ray_collide(Vector2(0, max_distance).rotated(_angular_delta * i + global_rotation - _angle_half))
		if col:
